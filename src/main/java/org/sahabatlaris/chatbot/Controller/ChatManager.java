...
public class ChatManager {
    ...
    /** Tampilkan kartu status toko + hari libur saat user tanya */
    private void addStatusTokoCard() {
        ...
        statusHeader.setPadding(new Insets(14, 16, 10, 16));
        statusHeader.setStyle(buka
            ? "-fx-background-color: #f0fdf4; -fx-background-radius: 16 16 0 0;"
            : "-fx-background-color: #fff5f5; -fx-background-radius: 16 16 0 0;");

        Label ikonStatus = new Label(buka ? "✅" : "❌");
        ikonStatus.setStyle("-fx-font-size: 20px;");

        VBox statusInfo = new VBox(2);
        Label statusLbl = new Label("Toko " + (buka ? "BUKA" : "TUTUP") + " Sekarang");
        statusLbl.setStyle("-fx-font-size: 14px; -fx-font-weight: bold; -fx-text-fill: " +
                (buka ? "#166534;" : "#991b1b;"));
        Label ketLbl = new Label(status[1]);
        ketLbl.setStyle("-fx-font-size: 11px; -fx-text-fill: #888;");
        statusInfo.getChildren().addAll(statusLbl, ketLbl);

        statusHeader.getChildren().addAll(ikonStatus, statusInfo);

        // Jam operasional list
        VBox jamSection = new VBox(4);
        jamSection.setPadding(new Insets(12, 16, 4, 16));
        Label jamTitle = new Label("🕐  Jam Operasional");
        jamTitle.setStyle("-fx-font-size: 12px; -fx-font-weight: bold; -fx-text-fill: #444;");
        jamSection.getChildren().add(jamTitle);
        ...
        for (String[] jam : jamList) {
            ...
            // Highlight hari ini
            String hariIni = LocalDate.now().getDayOfWeek()
                .getDisplayName(java.time.format.TextStyle.FULL,
                    new java.util.Locale("id", "ID"));
            boolean isToday = namaHari.equalsIgnoreCase(hariIni);

            HBox jamRow = new HBox();
            jamRow.setAlignment(Pos.CENTER_LEFT);
            jamRow.setPadding(new Insets(3, 0, 3, 0));

            Label hariLbl = new Label(namaHari);
            hariLbl.setMinWidth(90);
            hariLbl.setStyle(isToday
                ? "-fx-font-size: 12px; -fx-font-weight: bold; -fx-text-fill: #4B3FC8;"
                : "-fx-font-size: 12px; -fx-text-fill: #555;");

            Label jamLbl = new Label(bukaHari ? jam[2] + " – " + jam[3] : "Tutup");
            jamLbl.setStyle(bukaHari
                ? (isToday ? "-fx-font-size: 12px; -fx-text-fill: #4B3FC8; -fx-font-weight: bold;"
            ...
            if (isToday) {
                Label todayBadge = new Label("Hari ini");
                todayBadge.setStyle("-fx-background-color: #f0eeff; -fx-text-fill: #4B3FC8; -fx-font-size: 10px; -fx-padding: 2 7; -fx-background-radius: 10;");
                jamRow.getChildren().addAll(hariLbl, spacer, jamLbl, todayBadge);
                HBox.setMargin(todayBadge, new Insets(0, 0, 0, 8));
            } else {
                jamRow.getChildren().addAll(hariLbl, spacer, jamLbl);
            }
        ...
        // Hari libur section
        VBox liburSection = buildHariLiburSection();

        card.getChildren().addAll(statusHeader, jamSection, sep, liburSection);
    ...
    public void hapusRiwayatSidebar() {
        ...
        if (hasil.isPresent() && hasil.get() == ButtonType.OK) {
            ...
            muatSidebarRiwayat();
            addBotMessage("✅ Semua riwayat percakapan telah dihapus.");
        }
    }
